...
const WeatherWeekly = ({ weatherInfo }) => {
  ...
  useEffect(() => {
    if (weatherData?.list) {
      setList(weatherData.list);
    }
  }, [weatherData]);

  // Function to find day on consecutive date
              ...
              >
                ...
                <div className="bg-dark-b group-hover:bg-[#364a66] transition duration-400 ease-in-out">
                  <strong className="bg-dark-b group-hover:bg-[#364a66] transition duration-400 ease-in-out text-[#0095ff]">
                    {handleDay(item.dt_txt.split(" ")[0]) === "Today"
                      ? Math.round(list[0].main.temp_max - 273)
                      : Math.round(item.main.temp_max - 273)}
                  </strong>
                  /
                  {handleDay(item.dt_txt.split(" ")[0]) === "Today"
                    ? Math.round(list[0].main.temp_min - 273)
                    : Math.round(item.main.temp_max - 273) ===
